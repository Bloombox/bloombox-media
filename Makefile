...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-media" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-media"
endif
